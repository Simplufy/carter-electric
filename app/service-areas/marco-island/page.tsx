...
export default function MarcoIslandPage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-turquoise-900">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimatedSection>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={() => setIsContactOpen(true)} className="inline-flex items-center justify-center space-x-2 bg-white text-cyan-600 hover:bg-slate-100 px-8 py-4 rounded-full font-semibold text-lg transition-all">
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {otherCities.map((city) => <Link key={city.slug} href={`/service-areas/${city.slug}`} className="bg-white hover:bg-cyan-50 px-6 py-3 rounded-full text-slate-700 hover:text-cyan-600 transition-colors shadow-sm">{city.name}</Link>)}
          </div>
        </div>
      </section>
