...
export default function FortMyersPage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-orange-900">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            <div className="flex items-center gap-2 mb-4">
              <MapPin className="w-5 h-5 text-orange-400" />
              <span className="text-orange-400 font-medium">{city.county}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => setIsContactOpen(true)} className="inline-flex items-center justify-center space-x-2 bg-orange-500 hover:bg-orange-600 text-white px-8 py-4 rounded-full font-semibold text-lg transition-all hover:shadow-xl">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimatedSection>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={() => setIsContactOpen(true)} className="inline-flex items-center justify-center space-x-2 bg-white text-orange-600 hover:bg-slate-100 px-8 py-4 rounded-full font-semibold text-lg transition-all">
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {otherCities.map((city) => <Link key={city.slug} href={`/service-areas/${city.slug}`} className="bg-white hover:bg-orange-50 px-6 py-3 rounded-full text-slate-700 hover:text-orange-600 transition-colors shadow-sm">{city.name}</Link>)}
          </div>
        </div>
      </section>
